...
import { getJob, queryDocument } from "@/lib/api";
import { Citation, JobResponse } from "@/lib/types";
...
export default function ChatPage() {
  ...
  const canAsk = useMemo(
    () => Boolean(job?.document_id) && job?.status === "completed" && !asking,
    [job, asking],
  );

  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!question.trim() || !job?.document_id || !canAsk) return;
    ...
    try {
      const response = await queryDocument(job.document_id, userMessage.content);
    ...
    <section className="mx-auto mt-6 max-w-4xl">
      <div className="rounded-3xl bg-white p-6 shadow-soft">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 pb-4">
          <div>
            <p className="text-xs uppercase tracking-wide text-slate-500">Document Chat</p>
        ...
        {loadingJob && (
          <div className="mt-6">
            <Loader label="Loading job context..." />
          </div>
        )}

        {!loadingJob && !job && (
          <p className="mt-6 text-sm text-slate-600">No job found for this session.</p>
        )}
            ...
            <div className="mt-6 h-[55vh] space-y-4 overflow-y-auto rounded-2xl border border-slate-200 bg-slate-50 p-4">
              {messages.length === 0 && (
                <p className="text-sm text-slate-500">
                  Ask your first question about <span className="font-medium">{job.filename}</span>.
                </p>
              )}
            ...
            <form onSubmit={handleSubmit} className="mt-4 space-y-3">
              <textarea
                ...
                placeholder={
                  canAsk
                    ...
                    : "Waiting for document to complete processing..."
                }
              ...
              <div className="flex items-center justify-between">
                <p className="text-xs text-slate-500">
                  {job.document_id ? `Document ID: ${job.document_id}` : "Document not ready yet"}
                </p>
                <button
                  type="submit"
